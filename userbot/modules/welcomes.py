...
@bot.on(zelda_cmd(outgoing=True, pattern=r"setwelcome(?: |$)(.*)"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"checkwelcome$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"rmwelcome$"))
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Command :** `{cmd}setwelcome` <pesan welcome> atau balas ke pesan ketik `.setwelcome`\
        \n  •  **Function : **Menyimpan pesan welcome digrup.\
        \n\n  •  **Command :** `{cmd}checkwelcome`\
        \n  •  **Function : **Check pesan welcome yang anda simpan.\
        \n\n  •  **Command :** `{cmd}rmwelcome`\
        \n  •  **Function : **Menghapus pesan welcome yang anda simpan."
        "\n\n  •  **Format Variabel yang bisa digunakan di setwelcome :**\
        `{mention}`, `{title}`, `{count}`, `{first}`, `{jlast}`, `{fullname}`, `{userid}`, `{username}`, `{my_first}`, `{my_fullname}`, `{my_last}`, `{my_mention}`, `{my_username}`\
    "
    }
)
